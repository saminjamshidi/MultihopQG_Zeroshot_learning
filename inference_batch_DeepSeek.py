...
SAVE_PATH = "./generated_questions_batched_deepseek.jsonl"
BATCH_SIZE = 8

# ========== Load Models ==========
print("Loading DeepSeek model and tokenizer...")
deepseek_tokenizer = AutoTokenizer.from_pretrained(model_id, use_fast=True)
if deepseek_tokenizer.pad_token is None:
    deepseek_tokenizer.pad_token = deepseek_tokenizer.eos_token
deepseek_model = AutoModelForCausalLM.from_pretrained(model_id, torch_dtype=torch.float16, device_map="auto")
...
def build_zero_shot_prompt(document, answer, entities=None):
    ...
    if entities:
        ...
        user_msg   = (
            "Generate a complex multi-hop question.\n\n"
            f"Context: {document}\n"
            f"Answer: {answer}\n"
            f"Important Entities: {entity_str}"
        )
    else:
        user_msg = (
            "Generate a complex multi-hop question.\n\n"
    ...
    # list-of-dicts expected by apply_chat_template
    messages = [
        {"role": "system",
         "content": "You are a helpful research assistant that writes Hotpot-style questions."},
        {"role": "user", "content": user_msg},
    ]
    return deepseek_tokenizer.apply_chat_template(
        messages,
...
with open(SAVE_PATH, "w") as out_file:
    for i in tqdm(range(0, len(test_data), BATCH_SIZE)):
        ...
        inputs_with = deepseek_tokenizer(prompts_with_entities, return_tensors="pt", padding=True, truncation=True).to(DEVICE)
        inputs_without = deepseek_tokenizer(prompts_without_entities, return_tensors="pt", padding=True, truncation=True).to(DEVICE)

        # Greedy decode; switch on sampling if you want diversity
        gen_kwargs = {"max_new_tokens": 64,
                    "do_sample": False,
                    "pad_token_id": deepseek_tokenizer.eos_token_id}

        with torch.no_grad():
            out_with     = deepseek_model.generate(**inputs_with, **gen_kwargs)
            out_without  = deepseek_model.generate(**inputs_without, **gen_kwargs)

        decoded_with    = deepseek_tokenizer.batch_decode(out_with,    skip_special_tokens=True)
        decoded_without = deepseek_tokenizer.batch_decode(out_without, skip_special_tokens=True)
        ...
        for j in range(len(decoded_with)):
            out_file.write(dumps({
                ...
                "generated_with_entities": decoded_with[j].strip(),
                "generated_without_entities": decoded_without[j].strip(),
